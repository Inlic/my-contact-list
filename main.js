...
function addContact(event) {
  ...
  let newNumber = form.phone.value.replace(/[^\d]/g,"");
  let contactNumber = newNumber.replace(/([\d]{3})([\d]{3})([\d]{4})/, "($1)-$2-$3");
  // @ts-ignore
  let eContact = document.getElementById("ecCheckbox").checked ? true: false;
  let contactId = generateId();
  let newContact = {name: contactName, phone: contactNumber, emergency_contact: eContact, id: contactId};
...
function drawContacts() {
 ...
 contacts.forEach(contact => {
   if(contact.emergency_contact === true){
     template += `
